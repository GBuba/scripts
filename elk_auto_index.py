...
# Чтение common_name из ConfigMap в k8s
...
if len(common_names) > 0:
    ...
    for name in common_names.split(','):
        ...
        try:
            # Попробуйте получить политику ILM
            ilm_policy_name = f'{common_name}-policy'
            response = es.ilm.get_lifecycle(name=ilm_policy_name)
            print(f"ILM policy '{ilm_policy_name}' exists.")
        except NotFoundError:
            # Исключение NotFoundError будет выброшено, если политика не найдена
            print(f"ILM policy '{ilm_policy_name}' does not exist.")
            try:
